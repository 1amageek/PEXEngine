...
struct CompareIRCommandTests {
    ...
    @Test func comparesIRAndWritesRegressionReport() throws {
        ...
        #expect(report.baseline.artifact.path == baselineURL.absoluteString)
        #expect(report.baseline.artifact.sha256.count == 64)
        #expect(report.baseline.artifact.byteCount > 0)
        #expect(report.candidate.artifact.path == candidateURL.absoluteString)
        #expect(report.candidate.artifact.sha256.count == 64)
