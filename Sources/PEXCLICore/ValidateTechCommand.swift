...
public struct ValidateTechCommand: Sendable {
    ...
    public func run() async throws {
        ...
        let validationIssues = strict ? TechnologyIRValidator().validate(ir) : []
        let issues = validationIssues.map { "[\($0.code)] \($0.message)" }

        if !issues.isEmpty {
            if jsonOutput {
                let encoder = JSONEncoder()
                encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
                let data = try encoder.encode(TechValidationResult(ir: ir, valid: false, issues: issues))
        ...
        if jsonOutput {
            let encoder = JSONEncoder()
            encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
            let data = try encoder.encode(TechValidationResult(ir: ir, valid: true, issues: []))
            print(String(data: data, encoding: .utf8) ?? "{}")
        } else {
...
struct TechValidationResult: Codable {
    ...
    init(ir: TechnologyIR, valid: Bool, issues: [String]) {
        self.valid = valid
